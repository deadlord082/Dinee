...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    @section('content')
      ...
      <!-- Favorites -->
      <div>
        <h2 class="text-xl text-dinee-secondary mx-5 pb-5">Les favoris du public</h2>
        <div class="flex gap-5 overflow-x-scroll px-5 pb-15 no-scrollbar">
          @foreach($favorites as $favorite)
            <div class="flex flex-col w-60 h-40 flex-shrink-0">
              <img class="w-full h-full object-cover rounded-xl shadow-md mb-2" src="{{ $favorite->image }}" alt="{{ $favorite->name }}">
              <h3>{{ $favorite->name }}</h3>
              <p class="flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-1 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
